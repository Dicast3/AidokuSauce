        ...
        for filter in filters {
            match filter {
                FilterValue::Sort { id, index, .. } if id == "order" => match index {
                    1 => {
                        response.comics.sort_by_key(|b| {
                            core::cmp::Reverse(b.title.as_deref().unwrap_or("").to_lowercase())
                        });
                    }
                    2 => {
                    ...
                    3 => {
                        response.comics.sort_by(|a, b| {
                            b.rating.unwrap_or(0.0).total_cmp(&a.rating.unwrap_or(0.0))
                        });
                    }
                    4 => {
